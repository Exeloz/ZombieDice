...
class TournamentReproduction(DefaultReproduction):
    ...
    def parse_config(cls, param_dict):
        return DefaultClassConfig(param_dict,
                                  [ConfigParameter('elitism', int, 0),
                                   ...
                                   ConfigParameter('winner_survival', int, 10)]
                                   )
    ...
    def reproduce(self, config, species, pop_size, generation):
        ...
        for stag_sid, stag_s, stagnant in self.stagnation.update(species, generation):
            if stagnant:
                ...
                for m in stag_s.members.values():
                    if winner is None or m.fitness > winner.fitness:
                        winner = m
        winner.set_last_time_winner(generation)
        ...
        for spawn, s in zip(spawn_amounts, remaining_species):
            ...
            previous_winners = [(id, m) for id, m in old_members if 
                m.get_last_time_winner() is not None and 
                self.reproduction_config.winner_survival > (generation - m.get_last_time_winner())]
            for i, m in previous_winners:
                new_population[i] = m
                spawn -= 1
